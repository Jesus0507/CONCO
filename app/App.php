...
class Iniciar_Sistema {
    ...
    public function Establecer_Controladores() {
    ...
    public function Cargar_Parametos($N_parametors) {
        $parametros = [];
        for ($i = 2;$i < $N_parametors;$i++) {
            array_push($parametros, $this->url[$i]);
        }
        $this->Establecer_Parametros($parametros);
        if ($this->Validar_Funcion() == true) {
    ...
    public function Establecer_Funciones() {
        $N_parametors = sizeof($this->url);
        if ($N_parametors > 1) {
            if ($N_parametors > 2) {
                $this->Cargar_Parametos($N_parametors);
            } else {
                $this->Cargar_Funciones();
            }
        } else {
