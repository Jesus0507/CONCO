    ...
    $.ajax({
        ...
        var tabla = $("#example1").DataTable({
            ...
            "columns": [{
                ...
                {
                    "data": function(data) {
                        return data.primer_nombre + " " + data
                            .primer_apellido;
                    },
                },
                {
        ...
        $(document).on("click", ".mensaje-eliminar", function() {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: BASE_URL +
                            "Sector_Agricola/Eliminar_Sector_Agricola",
                        type: "POST",
                        data: {
                            'id': id
                        }
        ...
        $(document).on('click', '.ver-popup', function() {
            ...
            $.ajax({
                type: "POST",
                url: BASE_URL +
                    "Sector_Agricola/Consultas_Sector_Agricola_Persona",
                data: {
                    "id": id
                }
            }).done(function(result) {
                var result = JSON.parse(result);
                $('#rubro_principal').val(result[0][
                    "rubro_principal"
                ]);

                $('#rubro_alternativo').val(result[0][
                    "rubro_alternativo"
                ]);

                $('#financiado').val(result[0][
                    "financiado"
                ]);

                $('#org_agricola').val(result[0][
                    "org_agricola"
                ]);

                document.getElementById("registro_INTI")
                    .selectedIndex = result[0][
                        "registro_INTI"
                    ];

                document.getElementById("constancia_productor")
                    .selectedIndex = result[0][
                        "constancia_productor"
                    ];

                document.getElementById("senial_hierro")
                    .selectedIndex = result[0][
                        "senial_hierro"
                    ];

                document.getElementById("agua_riego")
                    .selectedIndex = result[0][
                        "agua_riego"
                    ];

                document.getElementById("produccion_actual")
                    .selectedIndex = result[0][
                        "produccion_actual"
                    ];
        ...
        $(document).on('click', '.btnEditar', function() {
            ...
            $.ajax({
                ...
                var result = JSON.parse(result);
                $('#rubro_principal2').val(result[0][
                    "rubro_principal"
                ]);

                $('#rubro_alternativo2').val(result[0][
                    "rubro_alternativo"
                ]);

                $('#financiado2').val(result[0][
                    "financiado"
                ]);

                $('#org_agricola2').val(result[0][
                    "org_agricola"
                ]);

                document.getElementById("registro_INTI2")
                    .selectedIndex = result[0][
                        "registro_INTI"
                    ];

                document.getElementById("constancia_productor2")
                    .selectedIndex = result[0][
                        "constancia_productor"
                    ];

                document.getElementById("senial_hierro2")
                    .selectedIndex = result[0][
                        "senial_hierro"
                    ];

                document.getElementById("agua_riego2")
                    .selectedIndex = result[0][
                        "agua_riego"
                    ];

                document.getElementById("produccion_actual2")
                    .selectedIndex = result[0][
                        "produccion_actual"
                    ];
            ...
            $(document).on("click", "#enviar", function() {
                var datos = [
                    document.getElementById(
                        "cedula_persona2").value,
                    document.getElementById(
                        "area_produccion2").value,
                    document.getElementById(
                        "anios_experiencia2").value,
                    document.getElementById(
                        "rubro_principal2").value,
                    document.getElementById(
                        "rubro_alternativo2").value,
                    document.getElementById(
                        "registro_INTI2").selectedIndex,
                    document.getElementById(
                        "constancia_productor2")
                    .selectedIndex,
                    document.getElementById(
                        "senial_hierro2").selectedIndex,
                    document.getElementById("financiado2")
                    .value,
                    document.getElementById("agua_riego2")
                    .selectedIndex,
                    document.getElementById(
                        "produccion_actual2").selectedIndex,
                    document.getElementById("org_agricola2")
                    .value,
                    id
                ];
                ...
                $.ajax({
                        type: "POST",
                        url: BASE_URL +
                            "Sector_Agricola/Editar_Sector_Agricola",
                        data: {
                            datos: datos,
                        },
                    })
