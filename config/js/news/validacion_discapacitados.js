...
btn_guardar.onclick=function(){
	if(discapacidades.length==0){
		...
		$.ajax({
			...
			data:{"cedula":persona.value,"discapacidades":discapacidades}
		}).done(function(result){
			swal({
				type:"success",
				title:"Enhorabuena",
				text:"Se ha registrado las discapacidades de forma exitosa",
				timer:2000,
				showConfirmButton:false
			});

			setTimeout(function(){location.href=BASE_URL+"Discapacitados/Consultas"},1000);
		});
	}
}