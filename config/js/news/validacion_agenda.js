...
setTimeout(function(){obtener_calendario("vacio","vacio");},500);
...
function get_eventos(){
  $.ajax({
    type:"POST",
    url:BASE_URL+"Agenda/consultar_evento",
  }).done(function(result){
    var resultado= JSON.parse(result);
    for(var i=0;i<resultado.length;i++){
      console.log(resultado[i]['fecha']);
...
function guardar_eventos(){
  ...
  $.ajax({
    type:"POST",
    url:BASE_URL+"Agenda/nuevo_evento",
    data:{"evento":datos}
  }).done(function(result){
    if(result==1){
      ...
      $.ajax({
       ...
       var users=JSON.parse(result);
          ...
          setTimeout(function(){location.href=BASE_URL+"Agenda/Consultas";},1000);
      ...
      if(eventos_registrados[i]['ubicacion'].toLowerCase()==ubicacion_evento.value.toLowerCase()){
       if(!validacion_horas(eventos_registrados[i]['horas'])){
         ...
         horas.push(eventos_registrados[i]['horas']);
           ...
           // if(validacion_horas(eventos_registrados[i]['horas']) && eventos_registrados[i]['ubicacion'].toLowerCase() != ubicacion_evento.value.toLowerCase() ){
           // }
           // else{
           //  validar=false;
           //  horas.push(eventos_registrados[i]['horas']);
...
function validacion_horas(hora_evento){
  ...
  // if((hora_desde_evento.value>=inicio['inicio'] && hora_desde_evento.value<=fin['inicio']) && (hora_hasta_evento.value<=fin['fin'] || hora_hasta_evento.value>fin['fin'])){
  //   console.log("no entra");
  // }
  // else{console.log('si entra');}
...
function llenar_calendario(ultimo,mes,anio){
  ...
  for(var i=1;i<ultimo+1;i++){
    ...
    if(mes<10){
      console.log("enviando fecha:"+anio+"-0"+(mes + 1)+"-"+day);
      var clase_td=getClassTd(anio+"-0"+(mes + 1)+"-"+day,i);
    }
    else{
      console.log("enviando fecha:"+anio+"-"+(mes + 1)+"-"+day);
...
function add_day(element,fecha){
 ...
 var fecha_evento=new Date(anio_event,(mes_event-1),dia_event);
  ...
  //   console.log(fecha_evento+" - "+new Date());
  if(fecha_evento<new Date()){
    ...
    if(lista_fechas.length==0){
     ...
     element.onmouseover=function(){element.style.background="#00C428";}
     element.onmouseout=function(){element.style.background="#00C428";}
    ...
    if(existe==false){
      ...
      if(element.className=="calendar_ocupado_selected" || element.className=="calendar_ocupado"){
        ...
        element.onmouseover=function(){element.style.background="#00C428";}
        element.onmouseout=function(){element.style.background="#85D7CF";}
      }
      else{
        element.style.background="#C7F2EE";
        element.onmouseover=function(){element.style.background="#00C428";}
        element.onmouseout=function(){element.style.background="#C7F2EE";}
      }
    }
  }
...
if(eventos_registrados.length==0){
 ...
 for(var i=0;i<eventos_registrados.length;i++){
  ...
  console.log("fecha evento:"+eventos_registrados[i]["fecha"]+" / fecha parametro:"+fecha);

  if(eventos_registrados[i]['fecha']==fecha){
    cont++;
      //  console.log(eventos_registrados[i]);
    }
  }
...
//    console.log("actual :"+fecha_act+"  -  calendario: "+fecha);
